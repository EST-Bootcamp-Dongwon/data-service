...
> 이 파일이 이 모듈의 규칙 정본이다. 예전에는 상위 공통 레포 `quant-contract` 에 공통 규칙을
> 두고 여기엔 모듈 특화만 적었으나, 그 레포는 폐기됐다(2026-09-15) — 지금은 이 파일 하나가 전부다.
...
계획이었으나, 그 모듈을 개발하지 않기로 해(2026-09-15) **옮기지 않고 이 저장소에 남는다** (ADR-DS-0007).

⚠️ `app/services/research/` 24개 파일은 전부 이 레포에 있고 배포본 화면이 그것을 쓴다.
**검증 없이 지우지 않는다.**
...
- ⚠️ **`app/core/trading_calendar.py`는 공휴일을 모른다.** `weekday() < 5`로 주말만 거른다.
  ...
  ★ 이 파일은 하류 소비 코드의 **수직 배리어가 의존하는 정본**이 된다
  (예전 계획이던 `label-service` 는 개발하지 않기로 폐기됐다 — 2026-09-15).
