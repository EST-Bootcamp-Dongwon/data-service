...
★ 판정을 지어내지 않는다 (이 파일에서 가장 중요한 규칙)
    ...
    기술적 5신호(RSI·MACD·볼린저)의 `BUY` 를 여기 놓지 않는다. 그 계산을 담고 있던
    `knowledge/technical.py` 는 **M9 에서 지웠다** (N93) — 호출자가 0건이었고,
    그 모듈의 `signal_score` 는 **판정불가를 0점으로 세서** M7 루브릭 재작성(N39·N73)이
    없앤 결함을 그대로 갖고 있었다. 되살리려면 요약본 §13 을 읽어라.
