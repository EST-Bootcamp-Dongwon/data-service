...
TAGS_METADATA = [
    ...
    },
    {
        "name": "자료 수집",
        "description": (
            "여섯 갈래 중 **첫 갈래 — DART 공시·정기보고서**를 자동으로 받아 "
            "자료 보관함(`clip`)에 담는다 (ADR-DS-0020). 그 앞까지는 사람이 손으로 담는 "
            "길만 열려 있었다."
            "\n\n"
            "**종목 하나**는 이 API 로(3호출 · 1초), **유니버스 일괄**은 셸로 돌린다 — "
            "`python3 scripts/collect_dart.py --top 350`. 일괄에 HTTP 경로를 두지 않은 "
            "것은 진행률·중단 장치를 쓸 곳이 아직 하나도 없기 때문이다."
            "\n\n"
            "⚠️ **갱신 사슬(`/api/refresh`)과 다른 축이다.** 그쪽은 시세(SQLite)를 다루고 "
            "이쪽은 보관함(Postgres)에 담는다. 그래서 사슬에 얹지 않았다."
            "\n\n"
            "거절은 넷이다 — `503`(껐거나 · 키가 없거나 · 보관함에 못 붙는다) · "
            "`429`(오늘 예산을 다 썼다) · `422`(어휘 밖) · `404`(고유번호가 없다). "
            "`/status` 만은 **언제나 200** 이다 — 화면이 버튼을 잠글지 정하려면 이유를 "
            "받아 볼 수 있어야 한다."
        ),
    },
]
...
| 갈래 | 지금 상태 |
| --- | --- |
| 공시 (DART 정기·수시) | **자동 수집 가동중** (ADR-DS-0020) · 전용 화면 준비중 |
| 정기보고서 (사업·반기·분기 원문) | **자동 수집 가동중**(목록) · 원문 파싱은 다음 |
| 뉴스 | 인증키만 배치 · 미착수 — **다음 차례**. 링크·제목·출처·발행일까지만 담습니다 |
