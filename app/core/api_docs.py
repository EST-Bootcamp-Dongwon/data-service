...
TAGS_METADATA = [
    ...
    },
    {
        "name": "자료 갱신",
        "description": (
            "**로컬 전용 운영 API** 다 (ADR-DS-0017). 갱신 사슬 다섯 단계"
            "(수집 → 축약본 → 스냅샷 → 마스터 → Postgres)를 `invoke refresh` 와 "
            "**같은 정의**로 돌린다 — 사슬은 `app/services/refresh_job.py` 한 곳에만 적혀 있다. "
            "\n\n"
            "배포본에서도 세 경로가 **등록은 되지만** 실행은 `503` 으로 거절한다. "
            "배포본은 파일시스템이 읽기 전용이고 시세 원본이 아예 없으며, 산출물은 어차피 "
            "커밋을 거쳐야 닿는다. 감추지 않고 이유를 말하기 위해 경로는 남겨 둔다. "
            "\n\n"
