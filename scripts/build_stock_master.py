...
def build() -> dict:
    ...
    """
    # ⚠️ **여기는 읽기 표면(`STORE_BACKEND`)을 일부러 우회한다** (ADR-DS-0018).
    #    이 스크립트는 갱신 사슬(`refresh_job.STEPS`)의 **4단계**이고 Postgres 적재
    #    (`load_pg.py`)는 **5단계**다. 즉 지금 Postgres 를 읽으면 이번 회차 자료가 아직
    #    없어 **직전 회차** 마스터가 나온다 — 오류는 안 나고 날짜로만 드러난다.
    #    그리고 `invoke refresh --skip-pg` 는 DB 단계만 건너뛰므로, 여기가 Postgres 를
    #    보면 **DB 없이 도는 갱신이 통째로 깨진다.**
    #    이 우회는 S8(쓰기 경로 전환)이 SQLite 를 없앨 때 자연히 사라진다.
