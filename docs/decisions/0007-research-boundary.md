...
> ⚠️ **2026-09-15 갱신** — 이관 대상이던 `research-service` 는 개발하지 않기로 해 제거됐고,
> 상위 정본이던 `quant-contract` 도 폐기됐다. 이 ADR의 "이관은 보류" 결정은 이제
> "이관하지 않는다"로 굳어진다 — 경계표의 "간다"(research-service) 항목도 이 저장소에
> 그대로 남는다.
